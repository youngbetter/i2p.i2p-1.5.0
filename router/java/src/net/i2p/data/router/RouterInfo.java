...
public class RouterInfo extends DatabaseEntry {
    ...
    public RouterInfo(RouterInfo old, String caps) {
        this();
        setIdentity(old.getIdentity());
        //setPublished(old.getPublished());
        //setAddresses(old.getAddresses());
...
//        }
    ...
    public void setCapacities(String newCap) {
        if (_signature != null)
            throw new IllegalStateException();
        if(newCap != null)
            _options.setProperty(PROP_CAPABILITIES, newCap);
    }
    ...
    private void doValidate() {
        ...
        if (!_isValid) {
            ...
            log.debug("@@YOUNG sig verify success");
        }
    }
    ...
    public void readBytes(InputStream in, boolean verifySig) throws DataFormatException, IOException {
        ...
        if (verifySig) {
            if (type != SigType.EdDSA_SHA512_Ed25519) {
                System.out.println("EdDSA_SHA512_Ed25519");
                // This won't work for EdDSA
