    ...
    private void run2() {
        ...
        while (_isRunning && !_manager.isShutdown()) {
            ...
            try {
                ...
                _log.debug("@@YOUNG after listPools:" + pools);
                for (int i = 0; i < pools.size(); i++) {
                    TunnelPool pool = pools.get(i);
                    if (!pool.isAlive())
                        continue;
                    // @YOUNG count how many tunnels to build
                    int howMany = pool.countHowManyToBuild();
                    _log.debug("@@YOUNG countHowManyToBuild howMany=" + howMany);
                ...
                int allowed = allowed();
                _log.debug("@@YOUNG first allowed=" + allowed);
                ...
                _log.debug("@@YOUNG second allowed=" + allowed);
