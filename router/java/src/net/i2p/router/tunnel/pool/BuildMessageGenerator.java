...
abstract class BuildMessageGenerator {
    ...
    public static void createRecord(int recordNum, int hop, TunnelBuildMessage msg,
        ...
        if (peerKey != null) {
            ...
            if (isEC) {
                ...
                long _finished = System.currentTimeMillis();
                log.debug("@@YOUNG encryptECIESRecord spent:" + (_finished - _started) + " ms");
                ...
                long _finished = System.currentTimeMillis();
                log.debug("@@YOUNG encryptRecord spent:" + (_finished - _started) + " ms");
            }
        } else {
    ...
    public static void layeredEncrypt(I2PAppContext ctx, TunnelBuildMessage msg,
        ...
        Log _log = ctx.logManager().getLog(BuildMessageGenerator.class);
        int mtype = msg.getType();
        boolean isShort = mtype == ShortTunnelBuildMessage.MESSAGE_TYPE;
        ...
        _log.debug("@@YOUNG msg record count=" + msg.getRecordCount() + " order:" + msg);
        for (int i = 0; i < msg.getRecordCount(); i++) {
            ...
            _log.debug("@@YOUNG isInbound=" + cfg.isInbound() + " layered from " + (hop - 1) + " to " + stop);
