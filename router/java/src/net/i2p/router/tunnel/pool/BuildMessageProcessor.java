...
class BuildMessageProcessor {
    ...
    public BuildRequestRecord decrypt(TunnelBuildMessage msg, Hash ourHash, PrivateKey privKey) {
        ...
        if (isShort) {
            ...
            log.debug("@@YOUNG Build ChaCha20 decrypt=" + (afterDecrypt - beforeDecrypt) + " ms");
        } else {
            long beforeDecrypt = System.currentTimeMillis();
            ...
            long afterDecrypt = System.currentTimeMillis();
            log.debug("@@YOUNG Build aes decrypt=" + (afterDecrypt - beforeDecrypt) + " ms");
        }
