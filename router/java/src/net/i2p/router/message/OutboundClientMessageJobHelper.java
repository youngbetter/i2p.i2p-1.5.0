 ...
 ****/
    ...
    static GarlicMessage createGarlicMessage(RouterContext ctx, long replyToken, long expiration, PublicKey recipientPK,
        ...
        GarlicMessage msg;
        if (isECIES) {
            log.debug("@YOUNG createGarlicMessage buildECIESMessage, GarlicConfig:" + config);
            ...
            log.debug("@YOUNG createGarlicMessage buildMessage, GarlicConfig:" + config);
    ...
    private static GarlicConfig createGarlicConfig(RouterContext ctx, long replyToken, long expiration, PublicKey recipientPK,
        ...
        if (bundledReplyLeaseSet != null) {
            log.debug("@YOUNG createGarlicConfig bundledReplyLeaseSet");
...
//            _persistenceHelper.writeProfile(prof);
        ...
        if (log.shouldLog(Log.INFO))
            log.info("Creating garlic config [cloves=" + config.getCloveCount() + "] to be encrypted to " + recipientPK
 ...
 ****/
    ...
    /**
     * Build a clove that stores the RouterInfo locally
     */
    private static PayloadGarlicConfig buildRouterInfoClove(RouterContext ctx, long expiration, RouterInfo storeRI) {
        DatabaseStoreMessage msg = new DatabaseStoreMessage(ctx);
        msg.setEntry(storeRI);
        msg.setMessageExpiration(expiration);
        DeliveryInstructions instructions = new DeliveryInstructions();
        instructions.setDeliveryMode(DeliveryInstructions.DELIVERY_MODE_ROUTER);
        instructions.setRouter(ctx.routerHash());
