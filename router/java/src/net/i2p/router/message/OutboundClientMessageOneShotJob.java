...
public class OutboundClientMessageOneShotJob extends JobImpl {
    ...
    public OutboundClientMessageOneShotJob(RouterContext ctx, OutboundCache cache, ClientMessage msg) {
        ...
        _log.debug("@@YOUNG 1->overallExpiration=" + overallExpiration);
        if (overallExpiration > 0) {
            if (overallExpiration < 24 * 60 * 60 * 1000l) {
                ...
                _log.debug("@@YOUNG 2->overallExpiration=" + overallExpiration);
            }
        ...
        _log.debug("@@YOUNG 3->overallExpiration=" + overallExpiration);
    }
    ...
    private int getNextLease() {
        ...
        _log.debug("@@YOUNG ourKeys: " + ourKeys + " supported: " + supported + " _encryptionKey: " + _encryptionKey);
 ...
 ***/
    ...
    private void send() {
        ...
        boolean needTags = GarlicMessageBuilder.needsTags(getContext(), _encryptionKey, _from.calculateHash(), tagsRequired);
        _log.debug("@@YOUNG _wantACK=" + _wantACK + " shouldRequestReply=" + shouldRequestReply + " needTags=" + needTags);
        boolean wantACK = _wantACK || shouldRequestReply || needTags;
        ...
        if (wantACK && _encryptionKey.getType() == EncType.ECIES_X25519) {
            _log.debug("@@YOUNG wantACK && _encryptionKey.getType() == EncType.ECIES_X25519");
        ...
        _log.debug("@@YOUNG garlic message info: " + msg + " wantACK:" + wantACK);
        if (msg == null) {
    ...
    private PayloadGarlicConfig buildClove() {
        ...
        PayloadGarlicConfig clove = new PayloadGarlicConfig(Certificate.NULL_CERT,
            ...
            expires,
            instructions, msg);
        _log.debug("@@YOUNG |payload|=" + p.getSize() + " |dataMsg|=" + msg.getSize() + " |clove|=" + clove);
        // defaults
 ...
 ****/
    ...
    private class DispatchJob extends JobImpl {
        ...
        public void runJob() {
            ...
            // @YOUNG
            // 把GarlicMessage 转化为TunnelDataMessage
            _log.debug("@@YOUNG before dispatch, message info:" + _msg);
