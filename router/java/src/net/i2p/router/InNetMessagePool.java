...
public class InNetMessagePool implements Service {
    ...
    public int add(I2NPMessage messageBody, RouterIdentity fromRouter, Hash fromRouterHash) {
        ...
        if (_log.shouldDebug())
            _log.debug("InNetMessagePool Rcvd"
        ...
        switch (type) {
            case TunnelGatewayMessage.MESSAGE_TYPE:
                _log.debug("TunnelGatewayMessage.MESSAGE_TYPE");
                shortCircuitTunnelGateway(messageBody);
                allowMatches = false;
                break;

            case TunnelDataMessage.MESSAGE_TYPE:
                _log.debug("TunnelDataMessage.MESSAGE_TYPE");
                ...
                if ((type > 0) && (type < _handlerJobBuilders.length)) {
                    ...
                    if (_log.shouldLog(Log.DEBUG))
                        _log.debug("Add msg to the pool - builder: " + builder
                            + " type: " + type + " " + messageBody.getClass().getSimpleName());
        ...
        if (allowMatches) {
            ...
            _log.debug("@YOUNG allowMatches handleReplies:" + replies);
    ...
    private void shortCircuitTunnelData(I2NPMessage messageBody, Hash from) {
        if (DISPATCH_DIRECT) {
            _log.debug("@@YOUNG DISPATCH_DIRECT");
            doShortCircuitTunnelData(messageBody, from);
        } else {
            _log.debug("@@YOUNG NOT DISPATCH_DIRECT");
